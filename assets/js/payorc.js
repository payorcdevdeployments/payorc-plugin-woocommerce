    ...
    var PayOrc = {
      ...
      openModal: function (iframeUrl) {
        ...
        var loader = $("<div/>", {
          ...
          '<img src="' +
            payorc_params.plugin_url +
            'assets/images/spinner-loader.gif" width="60px" height="60px" />'
        );
        ...
        $(document).on(
          "click",
          "#payorc-modal .modal-content span.close",
          function (e) {
            $("#payorc-modal").hide();
            $("#payorc-loader").hide();
            $("#payorc-iframe").attr("src", "");
            ...
            try {
              ...
              if (typeof e.data === "string") {
                ...
                if (!(trimmed.startsWith("{") && trimmed.endsWith("}"))) {
                  console.warn("Skipping non-JSON message:", e.data);
                  return;
                }
                fixedData = trimmed
                  .replace(/"parameters":"(\[.*?\])"/g, function (match, p1) {
                    return `"parameters":"${p1
                      .replace(/\\/g, "\\\\")
                      .replace(/"/g, '\\"')}"`;
                  })
                  .replace(/"custom_data":"(\[.*?\])"/g, function (match, p1) {
                    return `"custom_data":"${p1
                      .replace(/\\/g, "\\\\")
                      .replace(/"/g, '\\"')}"`;
                  })
              ...
              try {
                ...
                console.warn("Skipping message, not valid JSON:", fixedData);
                return;
              }
